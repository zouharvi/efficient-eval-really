...
args.add_argument("--reuse-results", action="store_true", help="Reuse results from computed/01-results_all.json if it exists")
...
if os.path.exists("computed/01-results_all.json") and args.reuse_results:
...
for method_name, method_fn in tqdm.tqdm(list(efficient_eval_really.methods.METHODS_BUDGETS.items())):
    ...
    for data_name, data in tqdm.tqdm(DATA_FN.items()):
        ...
        if args.reuse_results and data_name in results_out[method_name]:
            continue
